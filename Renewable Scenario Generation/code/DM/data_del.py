...
dataall=np.load(path+r'\dataall.npy')
#%%
...
dataall_delete=[]
for i in range(17437):
    s=data_check(dataall[i,])
    if s==0:
        dataall_delete.append(dataall[i,])
    print(i)
#%%
dataall_delete=np.array(dataall_delete)
dataall_delete=dataall_delete[:15000,:]
np.save(path+r'\dataall_delete.npy',dataall_delete)
